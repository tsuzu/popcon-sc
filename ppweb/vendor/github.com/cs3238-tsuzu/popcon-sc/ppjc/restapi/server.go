...
// NewServer creates a new api popcon s c ranking and submission manager server but does not configure it
func NewServer(api *swagger.PopconSCRankingAndSubmissionManagerAPI) *Server {
	s := new(Server)
...
// Server for the popcon s c ranking and submission manager API
type Server struct {
	...
	SocketPath    flags.Filename `long:"socket-path" description:"the unix socket to listen on" default:"/var/run/popcon-s-c-ranking-and-submission-manager.sock"`
	...
	api          *swagger.PopconSCRankingAndSubmissionManagerAPI
...
func (s *Server) SetAPI(api *swagger.PopconSCRankingAndSubmissionManagerAPI) {
	if api == nil {
		s.api = nil
		s.handler = nil
		return
...
func (s *Server) Serve() (err error) {
	...
	if s.hasScheme(schemeUnix) {
		...
		wg.Add(1)
		s.Logf("Serving popcon s c ranking and submission manager at unix://%s", s.SocketPath)
		go func(l net.Listener) {
			defer wg.Done()
			if err := domainSocket.Serve(l); err != nil {
				s.Fatalf("%v", err)
			}
			s.Logf("Stopped serving popcon s c ranking and submission manager at unix://%s", s.SocketPath)
		}(s.domainSocketL)
	}
	...
	if s.hasScheme(schemeHTTP) {
		...
		wg.Add(1)
		s.Logf("Serving popcon s c ranking and submission manager at http://%s", s.httpServerL.Addr())
		go func(l net.Listener) {
			defer wg.Done()
			if err := httpServer.Serve(l); err != nil {
				s.Fatalf("%v", err)
			}
			s.Logf("Stopped serving popcon s c ranking and submission manager at http://%s", l.Addr())
		}(s.httpServerL)
	}
	...
	if s.hasScheme(schemeHTTPS) {
		...
		wg.Add(1)
		s.Logf("Serving popcon s c ranking and submission manager at https://%s", s.httpsServerL.Addr())
		go func(l net.Listener) {
			defer wg.Done()
			if err := httpsServer.Serve(l); err != nil {
				s.Fatalf("%v", err)
			}
			s.Logf("Stopped serving popcon s c ranking and submission manager at https://%s", l.Addr())
