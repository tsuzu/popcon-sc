...
func (rm *RedisManager) JudgeProgressUpdate(cid, sid int64, status string) error {
	conn := rm.pool.Get()
	defer conn.Close()

	if err := conn.Send("SET", "judging_progress_"+strconv.FormatInt(cid, 10)+"_"+strconv.FormatInt(sid, 10), status); err != nil {
		return err
	}
...
func (rm *RedisManager) JudgeProgressGet(cid, sid int64) (string, error) {
	conn := rm.pool.Get()
	defer conn.Close()

	return redis.String(conn.Do("GET", "judging_progress_"+strconv.FormatInt(cid, 10)+"_"+strconv.FormatInt(sid, 10)))
}

func (rm *RedisManager) JudgeProgressDelete(cid, sid int64) error {
	conn := rm.pool.Get()
	defer conn.Close()

	if err := conn.Send("DEL", "judging_progress_"+strconv.FormatInt(cid, 10)+"_"+strconv.FormatInt(sid, 10)); err != nil {
		return err
	}
